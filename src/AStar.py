...
def AStar(graph, start, des, hValue):
    inFrontier = dict()
    ...
    node = (hValue[start], start, None) # node = (path cost, state, state's predecessor for finding path)
    explored = []
        ...
        for child_state in child_state_list:
            if not visited[child_state[0]]:
                if inFrontier[child_state[0]]:
                    tup = (child_state[1] + node[0] - hValue[node[1]] + hValue[child_state[0]], child_state[0], node[1])
                    temp = []
                    while frontier.queue:
                        temp.append(frontier.get())
                    for item in temp:
                        if item[1] == child_state[0]:
                            if item[0] > child_state[1] + node[0] - hValue[node[1]] + hValue[child_state[0]]:
                                item = tup
                        frontier.put(item)
                else:
                    frontier.put((child_state[1] + node[0] - hValue[node[1]] + hValue[child_state[0]], child_state[0], node[1]))
