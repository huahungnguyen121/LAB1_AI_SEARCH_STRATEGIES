...
def GBFS(graph, start, des, hValue):
    inFrontier = dict()
    ...
    while frontier.queue:
        ...
        for child_state in graph[node[1]]:
            if child_state != 0:
                child_state_list.append((nodeIndex, hValue[nodeIndex])) # (node, path cost)
            nodeIndex += 1
        for child_state in child_state_list:
            if not visited[child_state[0]]:
                if inFrontier[child_state[0]]:
                    tup = (child_state[1], child_state[0], node[1])
                    temp = []
                    while frontier.queue:
                        temp.append(frontier.get())
                    for item in temp:
                        if item[1] == child_state[0]:
                            if item[0] > child_state[1]:
                                item = tup
                        frontier.put(item)
                else:
                    frontier.put((child_state[1], child_state[0], node[1]))
